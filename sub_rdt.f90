...
!   ------------------PIPE GRID DIVISION----------------------------
      ...
      IF(MARK_SW_CHECK(I)==1)THEN
        ...
        jswr(i) = max(int(dst_sw(i)/20.0d0), 1)    ! 20M INTERVAL
				...
				! 20mぐらいの管に等しく分割
        dx_sw(i) = dst_sw(i)/dble(jswr(i))                               ! ?��E��E�궻�E�x


          j1 = 1
          j2 = j1 + 1
          dd = dx_sw(i)/2.0d0  ! ?�E��storm drain�E�궳�E?EE
					! １つめのマンホールと２つめのマンホールの距離？
          dseg = sqrt((x_sw(i, j2) - x_sw(i, j1))**2 + (y_sw(i, j2) - y_sw(i, j1))**2)  ! ?�땾�E??��E�?EE
  ...
  100     continue
          ...
          bs_sw(i, j) = bsdw_sw(i) + slp_sw(i)*(jswr(i) + 0.5d0 - j)*dx_sw(i)  ! ?�댆?��E�E���E�?�E?E,2,3,�E?jswr(i)�E?
...
110  ENDDO
    ...
    DO I=1, NUM-2
        I1=I ; I2=I+1
			! メッシュの図心と一番近いグリッドとの距離をJUDGEに入れる
    ...
    IF(MARK_SW_CHECK(I)/=1) GOTO 210
	    ...
	    IF(BS_SW(I,J)+ DD2_SW(I)  >= BASEO(ME)) GOTO 200	 ! FREEBORD FOR STORM DRAIN BOX = 1m
            IF(JUDGE== SQRT( (X_SD-X_M(I,J))**2 + (Y_SD-Y_M(I,J))**2)) THEN
						! メッシュ（me）から最も近い下水道管の番号？
            SW_I=INT(I)
						! メッシュ（me）から最も近い分割した下水道の番号？
            SW_J=INT(J)
